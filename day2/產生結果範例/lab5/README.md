...
### 或 2. 輸入下列提示詞，從本地端的 Project CodeGuard規則進行程式安全碼掃描

使用 `/software-security` Skill，進行掃描並儲存報表

```
/software-security 掃描這個專案，將掃描結果儲存為報表
```
...
### 3. 可以使用提示詞，要求 Claude Code 針對問題進行程式修正

輸入提示詞

```
修正 Critical 與 High 的問題，幫我修改程式碼
```

在本範例中，Claude Code修正查詢資料庫的語法並加入驗證
